#pragma config(Motor,  port2,           leftMotor,     tmotorServoContinuousRotation, openLoop, reversed)
#pragma config(Motor,  port3,           leftNine,      tmotorServoContinuousRotation, openLoop)
#pragma config(Motor,  port4,           rightMotor,    tmotorServoContinuousRotation, openLoop)
#pragma config(Motor,  port5,           rightNine,     tmotorServoContinuousRotation, openLoop, reversed)
#pragma config(Motor,  port6,           spinning1,     tmotorServoContinuousRotation, openLoop)
#pragma config(Motor,  port7,           spinning2,     tmotorServoContinuousRotation, openLoop, reversed)
#pragma config(Motor,  port8,           spinning3,     tmotorServoContinuousRotation, openLoop)
#pragma config(Motor,  port9,           spinning4,     tmotorServoContinuousRotation, openLoop, reversed)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

/*----------------------------------------------------------------------------------------------------*\
|*                           - Dual Joystick Control with Arm - 1 Remote -                            *|
|*                                      ROBOTC on VEX 2.0 Cortex                                      *|
|*                                                                                                    *|
|*  This program uses both the Left and the Right joysticks to run the robot using "tank control".    *|
|*  The Group 6 buttons on the top-right of the VEXnet Joystick are used to raise and lower an arm.   *|
|*                                                                                                    *|
|*                                        ROBOT CONFIGURATION                                         *|
|*    NOTES:                                                                                          *|
|*    1)  Ch1 is the X axis and Ch2 is the Y axis for the RIGHT joystick.                             *|
|*    2)  Ch3 is the Y axis and Ch4 is the X axis for the LEFT joystick.                              *|
|*                                                                                                    *|
|*    MOTORS & SENSORS:                                                                               *|
|*    [I/O Port]              [Name]              [Type]              [Description]                   *|
|*    Motor - Port 2          rightMotor           VEX Motor           Right motor                    *|
|*    Motor - Port 3          leftMotor            VEX Motor           Left motor                     *|
|*    Motor - Port 6          armMotor             VEX Motor           Arm motor                      *|
\*----------------------------------------------------------------------------------------------------*/

//+++++++++++++++++++++++++++++++++++++++++++++| MAIN |+++++++++++++++++++++++++++++++++++++++++++++++

task main()
{
  while(1 == 1)
  {
    // Driving Motor Control
    motor[leftMotor] = (int)(vexRT[Ch3] / 2);
    motor[rightMotor] = (int)(vexRT[Ch2] / 2);

    // Arm Control
    if(vexRT[Btn7U] == 1)
    {
      motor[leftNine] = -127;
      motor[rightNine] = -127;
    }
    else if(vexRT[Btn7D] == 1)
    {
      motor[leftNine] = 127;
      motor[rightNine] = 127;
    }
    if(vexRT[Btn8U] == 1)
  	{
    	motor[spinning1] = 127;
   		motor[spinning2] = 127;
   		motor[spinning3] = 127;
   		motor[spinning4] = 127;
   	}
   	else
    {
      motor[leftNine] = -12;
      motor[rightNine] = -12;
      motor[spinning1] = 0;
   		motor[spinning2] = 0;
   		motor[spinning3] = 0;
   		motor[spinning4] = 0;
    }
    delay(10);
  }
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
